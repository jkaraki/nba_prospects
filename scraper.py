...
player_links=[]
url_template_team = "http://www.barttorvik.com/team.php?team={parsed}&year=2008"
count = 0
for team in my_list:
    ...
    for b in links_with_text:
        if 'playerstat.php?year=2008&p' == b[:26]:
            player_links.append(b)


import csv
with open('player_urls_2008', 'w') as myfile:
...
new_data_2 = scrape_first(player_links)
new_data_3 = scrape_first(player_links)
new_data_4 = scrape_first(player_links)
new_data_5 = scrape_first(player_links)
merged_data = new_data+new_data_2

col=['Yr','Cls','Ht','Player','Team','Conf.',
        'G','Min%', 'PRPG!', 'ORtg', 'Usg', 'eFG', 'TS', 'OR',
        'DR','Ast', 'TO','Blk','Stl', 'FTR','FT_raw','FT_pct',
        '2P_raw', '2P_pct','3P_raw', '3P_pct', 'shit', 'shit', 'shit', 'shit' ]
df = pd.DataFrame(merged_data, columns = col)

df.to_csv('player_data_2018.csv')
